...
impl Graph {
    pub fn new() -> Graph {
        Graph {
            ...
            answer_count: 0,
            equalifier: ExactEqualifier::new(),
        }
    }
        ...
        match cmd.cmd {
            CommandType::Set => {
                ...
                if question.answer.is_some()
                    && question.confidence > self.influence_source_confidence
                {
                    // update source quality
                    // TODO put distance function here & clustering
                    let answer_distance =
                        self.equalifier.get_distance(&cmd.answer, &question.answer);
                    if answer_distance < 1 {
                        println!(
            ...
            CommandType::Get => {
                ...
                Ok(format!(
                    ...
                    question.name,
                    question.confidence,
                    question.answer.as_ref().unwrap_or(&default_answer)
                ))
